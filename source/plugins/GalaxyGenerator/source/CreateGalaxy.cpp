...
vector<Planet> CreateGalaxy::createPlanets(int planetCount) {
	// TODO: Consider separating this from the actual star creation in the future if I want to do multithreading
	// Steps:
...
Star CreateGalaxy::plotStar(int distanceProportionInt) {
	...
	int planetCount = calculatePlanetCount(myGalaxy.getStarTypeMeanPlanets(starTypeID));
	newStar.setPlanetCount(planetCount);
	vector<Planet> planetList = createPlanets(planetCount);
	newStar.setPlanetList(planetList);
	return newStar;
...
int CreateGalaxy::calculatePlanetCount(int mean) {
	int planetCount = static_cast<int>(starClusterGen.nextNormal(static_cast<double>(mean), mean / 2.0));
	if (planetCount < 0) {
		planetCount = 0;
	} else if (planetCount > 5 * mean) {
		planetCount = 5 * mean;
	}
